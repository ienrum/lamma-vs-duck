      ...
      <div className="flex items-center gap-4 justify-center">
        {["down", "right", "left", "up"].map((direction) => (
          <label key={direction} className={cn("text-sm font-sans", {
            "text-red-500": gameInfo.count(direction as Direction) === gameInfo.maxCount(direction as Direction) + 1
          })}>
            {direction} : {gameInfo.count(direction as Direction)} / {gameInfo.maxCount(direction as Direction) + 1}
          </label>
        ))}
      </div>
    </div>
